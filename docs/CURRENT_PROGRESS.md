...
## 控制端与被控端一键更新（已实现，待真实双节点验收）

一键更新现已由控制端与被控端共同实现：

- **控制端：** `rcctl update apply` 构建文件 SHA-256 清单、分块上传并触发更新；`--wait` 在 Agent 重启期间自动重连，`update status` 可恢复查询结果。
- **被控端：** 仅允许已配对控制会话创建更新；暂存区会校验分块、完整文件、文件路径、包大小、必备发布文件和版本降级，之后通过特权 Broker 运行更新任务。
- **数据与安全：** 更新任务记录审计事件，保持 `ProgramData` 中的配置、证书、配对关系、任务与审计数据；不接受路径穿越、篡改块或未经配对认证的请求。
- **失败恢复：** `Update-RemoteController.ps1` 会保存旧安装目录，安装失败时恢复旧文件并尝试重启旧服务；更新状态和安装任务 ID 保存在受保护的数据根中。
- **验收：** 已补充契约、暂存校验和脚本 WhatIf 测试；仍需在主机到 Hyper-V VM 上执行真实更新、断线重连、安装失败回滚和升级后配对保留验收。
